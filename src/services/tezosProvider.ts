...
const tezosProvider = (
  ...
  const signMessage = async (): Promise<any> => {
    try {
      ...
      const signedMessage = await tezos.signer.sign(
        Buffer.from("Hello World", "utf-8").toString("hex")
      );
  ...
  const signTransaction = async (): Promise<any> => {
    ...
    try {
      ...
      tezos.setSignerProvider(await InMemorySigner.fromSecretKey(keyPair?.sk));
      const signedTransaction = await tezos.signer.sign(
        fromObjectToHex(transaction)
      );
